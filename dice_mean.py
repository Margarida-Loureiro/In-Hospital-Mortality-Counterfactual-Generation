...
@dataclass
class DiceGeneticNativeMissingMeanWrapperConfig(CFExperimentConfig):
    out_name: str = "dice_genetic_native_missing_mean_wrapper"
    max_queries: int = 250
    total_cfs: int = 8
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run DiCE genetic counterfactuals with native-missing prediction wrapper and mean imputation.")
...
def select_ranked_mutable_features(cfg: DiceGeneticNativeMissingMeanWrapperConfig, metadata: pd.DataFrame, model_dir: Path) -> List[str]:
...
def build_dice_generation_kwargs(cfg: DiceGeneticNativeMissingMeanWrapperConfig) -> Dict[str, float]:
...
def dice_weight_args_summary(cfg: DiceGeneticNativeMissingMeanWrapperConfig) -> tuple[List[str], str]:
...
def main() -> None:
    ...
    cfg = DiceGeneticNativeMissingMeanWrapperConfig()
    args = parse_args()
    ...
    summary: Dict[str, object] = {
        "method": "dice_genetic_native_missing_mean_wrapper",
        ...
        "note": (
            "DiCE genetic with native-missing prediction wrapper: mean values are used only as "
